...
if(life_inv_rim >= 2) exitWith {hint "Voce nao pode carregar mais orgaos!"};//if you already have 2 kidneys, then go sell them already, no stockpiling
if(!([true,"rim",1] call life_fnc_handleInv)) exitWith {hint "Voce nao tem espaço para armazenar os orgaos!"};//if no room for kidney, you cannot take their kidney, duh, waste not want not
