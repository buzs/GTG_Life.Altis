...
if (life_is_processing) exitWith {hint "O colega está ocupado. Por favor, não se estresse!";};    
...
if(vehicle player != player) exitwith {hint format["Você não pode sentar no veículo"];};
...
{
	case "lsd": {[["flor","ether"],[2,2],"lsd",1,"lsd",3500];};
	default {[]};
};
...
if (_productionAmount < 1) exitWith {
	hint "Você não tem os itens para processar!";
...
if (_productionAmount < 1) exitWith {
	hint "Você não tem dinheiro suficiente para processar";
	life_is_processing = false;
};
...
_pgText ctrlSetText format["%2 (1%1)...","%","Processando."];
...
while{true} do
{
	...
	_pgText ctrlSetText format["%3 (%1%2)...",round(_cP * 100),"%","Processando."];
	if(_cP >= 1) exitWith {};
